...
exports.handleFbVerify = handleFbVerify;
exports.sendFbMessage = sendFbMessage;
exports.handleFbEvent = handleFbEvent;
const supabase_1 = require("./supabase");
const aiService_1 = require("./aiService");
const lineHandler_1 = require("./lineHandler");
async function handleFbVerify(req, res) {
    const verify_token = await (0, supabase_1.getSystemSetting)('fb_verify_token', process.env.FB_VERIFY_TOKEN || '');
...
async function sendFbMessage(senderId, text, imageUrl) {
    const PAGE_ACCESS_TOKEN = await (0, supabase_1.getSystemSetting)('fb_page_access_token', process.env.FB_PAGE_ACCESS_TOKEN || '');
...
async function getFbCustomerName(senderId) {
    const PAGE_ACCESS_TOKEN = await (0, supabase_1.getSystemSetting)('fb_page_access_token', process.env.FB_PAGE_ACCESS_TOKEN || '');
    if (!PAGE_ACCESS_TOKEN)
        return 'ไม่ทราบชื่อ (Facebook User)';
...
async function handleFbEvent(req, res) {
    const body = req.body;
    if (body.object !== 'page') {
        res.sendStatus(404);
        return;
    ...
    body.entry.forEach(async (entry) => {
        ...
        messagingEvents.forEach(async (webhook_event) => {
            ...
            if (webhook_event.message) {
                ...
                if (!userMessage) {
                    if (webhook_event.message.attachments) {
                        ...
                        await (0, lineHandler_1.notifyAdmin)('ลูกค้าส่งรูปภาพ (Facebook)', `ชื่อลูกค้า: ${customerName}\nFacebook Sender ID: ${senderId}`);
                    }
                    return;
                }
                ...
                if (shouldHandoff) {
                    ...
                    const customerName = await getFbCustomerName(senderId);
                    await (0, lineHandler_1.notifyAdmin)('พบข้อความที่ต้องโอนสาย (Facebook)', `ชื่อลูกค้า: ${customerName}\nข้อความ: "${userMessage}"`);
                    return;
                }
                ...
                let chatHistory = [];
                try {
                    const { data } = await supabase_1.supabaseAdmin.from('chat_sessions').select('history').eq('user_id', fbUserId).single();
